/**
 * This MultiSelect component from {@link: https://shadcn-extension.vercel.app/docs/multi-select}
 * I just modified something to fit my project
 */
...
import { createContext, forwardRef, useCallback, useContext, useState, useRef } from 'react'
...
const MultiSelector = ({ values: value, onValuesChange: onValueChange, loop = false, className, children, dir, ...props }) => {
  ...
  return (
    ...
    >
      {value.map((item, index) => (
        <Badge
          key={item.value}
          className={cn('flex items-center gap-1 rounded-xl px-2', activeIndex === index && 'ring-2 ring-muted-foreground')}
          variant={'secondary'}
        >
...
const MultiSelectorItem = forwardRef(({ className, value, children, ...props }, ref) => {
  ...
  const isIncluded = Options.some((opt) => opt.value === value)
  return (
    <CommandItem
      ref={ref}
      {...props}
      onSelect={() => {
        onValueChange({ value, label: children })
        setInputValue('')
      }}
      className={cn(
